...
def get_previous_messages(request):
    if request.method == "GET":
        ...
        for message in previous_messages:
            message_data = {'id': message.id, 'userid': message.user.id, 'code': message.code, 'message': message.message, 'timestamp': message.created_at.isoformat(), 'username': message.user.username, 'profile_pic': message.user.profile_image.url}
